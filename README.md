...
- 域树目录与全部表铺平目录
- 主键、字段备注、关系约束及复合外键映射详情；可选内容只在原 JSON 有值时显示
...
5. 在表详情点击“AI 审核此表”进入全屏标注台，查看完整对话、每次工具调用和返回结果、低置信字段及草稿摘要，也可以继续对话纠正。
...
Ontology 类文件包含 `class_name`、`description`、`aliases`、`attributes` 和空数组 `metrics`。RDB Mapping 包含 `class_name`、数据库表名及已筛选字段的属性名、数据库列名和固定类型。字段配置了 `enum_ref` 时，实体属性始终输出引用；只有当前标注同时包含枚举值定义时，才在同一 0 级域的 `enums` 目录生成文件，避免覆盖参考资料中已经存在的枚举定义。
